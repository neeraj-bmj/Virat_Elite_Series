  ...
  return (
    <div className='NavbarContainer w-full h-auto md:h-[15vh] bg-gray-800 text-white flex flex-col md:flex-row justify-between items-center sticky top-0 z-[1000] px-4 md:px-10 py-4 md:py-15'>
      ...
      <div className="NavLinkPart text-zinc-50 flex flex-wrap justify-center md:justify-end gap-3 md:gap-8 mt-1 md:mt-0 font-semibold text-lg md:text-2xl">
        <Magnet padding={50} disabled={false} magnetStrength={10}>
          <NavLink className={(e)=>e.isActive ? "text-rose-500 rounded-md" : ""} to={"/"}>Home</NavLink>
        </Magnet>
        <Magnet padding={50} disabled={false} magnetStrength={10}>
          <NavLink className={(e)=>e.isActive ? "text-rose-500 rounded-md" : ""} to={"/products/services-page"}>Products</NavLink>
        </Magnet>
        <Magnet padding={50} disabled={false} magnetStrength={10}>
          <NavLink className={(e)=>e.isActive ? "text-rose-500 rounded-md" : ""} to={"/about"}>About Us</NavLink>
        </Magnet>
        <Magnet padding={50} disabled={false} magnetStrength={10}>
          <NavLink className={(e)=>e.isActive ? "text-rose-500 rounded-md" : ""} to={"/contact"}>Contact Us</NavLink>
        </Magnet>
        <Magnet padding={50} disabled={false} magnetStrength={10}>
         {
          !loginUser ? (
              <NavLink className={(e)=>e.isActive ? "text-rose-500 rounded-md" : ""} to={"/login"}>Login</NavLink>
          ) : (
              <button onClick={()=> handleLogout()} className='px-2 py-1 font-semibold rounded-lg cursor-pointer bg-rose-500 hover:bg-rose-600 transition-all hover:scale-105 duration-300 ease-in-out'>Logout</button>
          )
         }
        </Magnet>
      </div>
